...
def scattering2d(x, pad, unpad, backend, J, L, phi, psi, max_order,
    ...
    if phi[0].device is not U_0_c.device: #dm
        phi[0] = phi[0].to(U_0_c.device)
    ...
    for n1 in range(len(psi)):
        ...
        if psi[n1][0].device is not U_0_c.device: #dm
            psi[n1][0] = psi[n1][0].to(U_0_c.device)
        ...
        if phi[j1].device is not U_1_c.device: #dm
            phi[j1] = phi[j1].to(U_1_c.device)
        ...
        for n2 in range(len(psi)):
            ...
            if psi[n2][j1].device is not U_1_c.device: #dm
                psi[n2][j1] = psi[n2][j1].to(U_1_c.device)
            ...
            if phi[j2].device is not U_2_c.device: #dm
                phi[j2] = phi[j2].to(U_2_c.device)
