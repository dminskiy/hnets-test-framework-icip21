...
class Experiments:
    ...
    def dict_to_graph(self, out_dir, out_name, subplots):
        ...
        k2_results_dict = self.map_results(target_k=2)

        dataset = self.dict["Experiment0"]["dataset"]

        ks_dicts = {2:k2_results_dict}

        datas = {}

        for data_partition in k2_results_dict:
        ...
        if subplots == 4:
            ...
            fig, axs = plt.subplots(nrows=1, ncols=4, figsize=(20, 6), dpi=150, subplot_kw=dict(projection='radar'))
            plots = {
                    100: {
                        2: axs[0]
                    },
